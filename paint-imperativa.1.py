...
def iniciar_figura_nova(event):
    ...
    if tipo == 'linha':
        ...
        figura_nova = ('circulo', (event.x, event.y, event.x, event.y, event.x, event.y))
...
def atualizar_figura_nova(event):
    ...
    if not figura_nova:
        ...
        raio = math.sqrt((event.x - x1) ** 2 + (event.y - y1) ** 2)
        figura_nova = ('circulo', (x1 - raio, y1 - raio, x1 + raio, y1 + raio))
